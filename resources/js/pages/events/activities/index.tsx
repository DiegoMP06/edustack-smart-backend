...
export default function Activities({ event, activities, filter }: ActivitiesProps) {
    return (
        <AppLayout breadcrumbs={breadcrumbs(event)} collectionName='activities' withSearch>
            <Head title={`Actividades de ${event.name}`} />

            <EventLayout event={event}>
                <div className="mb-15">
                    <Button onClick={() => router.visit(events.activities.create(event.id))}>
                        <Plus />
                        Crear Actividad
                    </Button>
                </div>
                ...
                {activities.data.length > 0 ? (
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        {activities.data.map((activity) => (
                            <ActivityItem key={activity.id} activity={activity} eventId={event.id} />
                        ))}
                    </div>
                ) : (
