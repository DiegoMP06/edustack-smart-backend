...
const VARIANTS = {
    default: {
        wrap: 'divide-y divide-border border border-border rounded-lg overflow-hidden',
        item: '',
    },
    separated: {
    ...
    bordered: { wrap: 'divide-y-2 divide-indigo-200', item: '' },
};
                    ...
                    return (
                        <div key={i} className={v.item}>
                            <button
                                ...
                                className="flex w-full items-center justify-between px-4 py-3 text-left font-semibold text-foreground transition-colors hover:bg-accent"
                            >
                                <span>{item.question}</span>
                                <span
                                    className={`ml-2 shrink-0 text-lg leading-none transition-transform duration-200 ${isOpen ? 'rotate-45' : ''}`}
                                    aria-hidden="true"
                                >
                                    +
                                </span>
...
export const AccordionConfig: ComponentConfig<AccordionProps> = {
    ...
    fields: {
        ...
        items: {
            ...
            getItemSummary: (item) =>
                (item as AccordionItem).question || 'Nueva pregunta',
        },
    },
    defaultProps: {
