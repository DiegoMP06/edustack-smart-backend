...
#[Signature('make-module:crud-simple
    {module     : Module name (e.g. Blog)}
    {model      : Model name (e.g. Post)}
    {--requests : Generate form requests}
    {--routes   : Generate module routes file and register it}
    {--policy   : Generate policy with Spatie roles}
    {--resources : Generate API resource and collection}
    {--queries  : Generate query filter class}
    {--api-queryable : Use ApiQueryable-based query filter stub}
    {--media-resource : Use MediaResource-based resource stub}
    {--with-model : Generate model with #[Fillable] and SoftDeletes}
    {--content : Generate content controller and UpdateContentRequest}
    {--status : Generate invokable status controller}
    {--media : Generate media gallery controller}
    {--features-routes : Generate feature routes (content/status/media)}
    {--rest-routes : Generate REST routes snippet}
    {--force : Overwrite existing files}
    {--dry-run : Preview changes without writing files}')]
#[Description('Generate a full module CRUD structure (DTO, Actions, Service, Controller)')]
class MakeModuleCrud extends Command
{
    use GeneratesModuleFiles;

    protected $aliases = ['mm:crud-simple', 'mm:crud'];
    ...
    public function handle(): void
    {
        ...
        if ($this->option('features-routes')) {
            $this->generateFeatureRoutes();
        }

        if ($this->option('rest-routes')) {
            $this->generateRestRoutes();
        }

        $this->summary("Module <fg=cyan>{$this->module} / {$this->model}</> generated.");
    }
